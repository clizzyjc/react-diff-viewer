...
class DiffViewer extends React.Component {
    constructor(props) {
        ...
        this.renderDiff = () => {
            ...
            console.log("matches = ", occurrence);
            ...
            for (var i = 0; i < occurrence; i++) {
                ...
                console.log("```body_index", body_index);
                console.log("```next_timestamp_index", next_timestamp_index);
                if (next_timestamp_index < body_index) {
                    console.log("for the index{", i, "}, content {} of Body not found. Found a { after indicated index");
                    result[i] = null;
                }
                else if (body_index < 0) {
                    console.log("for the index{", i, "}, content {} of Body not found. No { found after indicated index");
                    result[i] = null;
                }
                else {
                    result[i] = content.substring(body_index, content.indexOf("[2020", content.indexOf(body_pattern_to_find) + 1) - 1);
                }
                console.log("jappy", result[i]);
            ...
            const { lineInformation, diffLines } = compute_lines_1.computeLineInformation(oldValue, newValue, disableWordDiff, compareMethod, listofErrors, concatenatedStr, schemaContent);
